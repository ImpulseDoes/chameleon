...
export class TongueStore {
  ...
  constructor(options?: StoreOptions) {
    ...
    this.roles = new Tongue<string, Role>(options?.roles ?? 1000)
    this.channels = new Tongue<string, Channel>(options?.channels ?? 1000)
    this.members = new Tongue<string, Member>(options?.members ?? 1000)
    this.messages = new Tongue<string, Message>(options?.messages ?? 100)
    this.users = new Tongue<string, User>(options?.users ?? 2000)
    this.emojis = new Tongue<string, Emoji>(options?.emojis ?? 1000)
    this.stickers = new Tongue<string, Sticker>(options?.stickers ?? 1000)
    ...
    this.voiceStates = new Tongue<string, Voice>(options?.voiceStates ?? 1000)
